...
#'
#' @export
scqe = function(post=NA, treatment=NA, outcome=NA, delta=NA, cohort=NA, untr_pre=NA,untr_post=NA,tr_post=NA,tr_pre=NA,Y_tr_post=NA, Y_untr_post=NA,Y_tr_pre=NA,Y_untr_pre=NA, untr=NA,tr=NA,Y_tr=NA,Y_untr=NA, min_delta=NA, max_delta=NA,...){
  scqe.obj <- NA
...
scqe.1cfull = function(treatment, outcome, delta, ...){
  ...
  for(i in 1:length(delta)){
    ...
    r[i,] <- c(delta[i], Beta_SCQE_1C, Beta_SCQE_1C - 1.96*SE_B_SCQE_1C, Beta_SCQE_1C + 1.96*SE_B_SCQE_1C,SE_B_SCQE_1C)
...
scqe.2csumm <- function(untr_pre,untr_post,tr_post,tr_pre,Y_tr_post,
  ...
  SCQE_2C_df <- data.frame(term = delta_list, estimate = Beta_SCQE,
                           conf.low = Beta_SCQE - 1.96*SE_B_SCQE, conf.high = Beta_SCQE + 1.96*SE_B_SCQE,se = SE_B_SCQE)
...
scqe.1csumm <- function(untr_1C, Y_untr_1C, tr_1C, Y_tr_1C, min_delta, max_delta, ...){
  ...
  SCQE_1C_df <- data.frame(term = outcome_list, estimate = Beta_SCQE_1C,
                           conf.low = Beta_SCQE_1C - 1.96*SE_B_SCQE_1C, conf.high = Beta_SCQE_1C + 1.96*SE_B_SCQE_1C,se = SE_B_SCQE_1C)
